    ...
    public class Cell : IComparable<Cell>
    {
        ...
        public int ColumnNumber
        {
            ...
            set
            {
                ...
                {
                    throw new ExcelRangeException("OutOfRangeException", "The passed column number (" + value + ") is out of range. Range is from " + Worksheet.MIN_COLUMN_NUMBER + " to " + Worksheet.MAX_COLUMN_NUMBER + " (" + (Worksheet.MAX_COLUMN_NUMBER + 1) + " rows).");
                }
                columnNumber = value;
            }
        }
        ...
        public int RowNumber
        {
            ...
            set
            {
                ...
                {
                    throw new ExcelRangeException("OutOfRangeException", "The passed row number (" + value + ") is out of range. Range is from " + Worksheet.MIN_ROW_NUMBER + " to " + Worksheet.MAX_ROW_NUMBER + " (" + (Worksheet.MAX_ROW_NUMBER + 1) + " rows).");
                }
                rowNumber = value;
            }
        }
        ...
        public Style SetStyle(Style style, bool unmanaged = false)
        {
            if (style == null)
            {
                throw new ExcelStyleException("A reference is missing in the style definition", "No style to assign was defined");
            }
        ...
        public static string ResolveCellAddress(int column, int row, AddressType type = AddressType.Default)
        {
            ...
            {
                throw new ExcelRangeException("OutOfRangeException", "The column number (" + column + ") is out of range. Range is from " + Worksheet.MIN_COLUMN_NUMBER + " to " + Worksheet.MAX_COLUMN_NUMBER + " (" + (Worksheet.MAX_COLUMN_NUMBER + 1) + " columns).");
            }
            switch (type)
            {
        ...
        public static string ResolveColumnAddress(int columnNumber)
        {
            ...
            {
                throw new ExcelRangeException("OutOfRangeException", "The column number (" + columnNumber + ") is out of range. Range is from " + Worksheet.MIN_COLUMN_NUMBER + " to " + Worksheet.MAX_COLUMN_NUMBER + " (" + (Worksheet.MAX_COLUMN_NUMBER + 1) + " columns).");
            }
            // A - XFD
            int j = 0;
            int k = 0;
        ...
        public static void ValidateColumnNumber(int column)
        {
            if (column > Worksheet.MAX_COLUMN_NUMBER || column < Worksheet.MIN_COLUMN_NUMBER)
            {
                throw new ExcelRangeException("A general range exception occurred", "The column number (" + column + ") is out of range. Range is from " +
        ...
        public static void ValidateRowNumber(int row)
        {
            ...
            {
                throw new ExcelRangeException("A general range exception occurred", "The row number (" + row + ") is out of range. Range is from " +
