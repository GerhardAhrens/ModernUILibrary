namespace ModernBaseLibrary.Core
{
    ...
    public class TypeMap : IDictionary<string, TypeMapEntry>
    {
        ...
        {
            if(this._map.TryGetValue(name, out TypeMapEntry entry))
            {
            ...
            {
                this._map[name] = new TypeMapEntry(name, type);
            }
        }
        ...
        {
            ((IDictionary<string, TypeMapEntry>)this._map).Add(key, value);
        }

        public bool ContainsKey(string key)
        {
            return ((IDictionary<string, TypeMapEntry>)this._map).ContainsKey(key);
        }

        public bool Remove(string key)
        {
            return ((IDictionary<string, TypeMapEntry>)this._map).Remove(key);
        }

        public bool TryGetValue(string key, [MaybeNullWhen(false)] out TypeMapEntry value)
        {
            return ((IDictionary<string, TypeMapEntry>)this._map).TryGetValue(key, out value);
        }

        public void Add(KeyValuePair<string, TypeMapEntry> item)
        {
            ((ICollection<KeyValuePair<string, TypeMapEntry>>)this._map).Add(item);
        }

        public void Clear()
        {
            ((ICollection<KeyValuePair<string, TypeMapEntry>>)this._map).Clear();
        }

        public bool Contains(KeyValuePair<string, TypeMapEntry> item)
        {
            return ((ICollection<KeyValuePair<string, TypeMapEntry>>)this._map).Contains(item);
        }

        public void CopyTo(KeyValuePair<string, TypeMapEntry>[] array, int arrayIndex)
        {
            ((ICollection<KeyValuePair<string, TypeMapEntry>>)this._map).CopyTo(array, arrayIndex);
        }

        public bool Remove(KeyValuePair<string, TypeMapEntry> item)
        {
            return ((ICollection<KeyValuePair<string, TypeMapEntry>>)this._map).Remove(item);
        }

        public IEnumerator<KeyValuePair<string, TypeMapEntry>> GetEnumerator()
        {
            return ((IEnumerable<KeyValuePair<string, TypeMapEntry>>)this._map).GetEnumerator();
        }
