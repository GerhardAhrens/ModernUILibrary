//-----------------------------------------------------------------------
// <copyright file="DeepConvert.cs" company="Lifeprojects.de">
//     Class: DeepConvert
...
namespace ModernBaseLibrary.Core
{
    ...
    public static partial class DeepConvert
    {
        ...
        public static T ChangeType<T>(object value) => (T)ChangeType(value, typeof(T), new DeepConvertSettings());
        ...
        /// <returns>An object whose type is <typeparamref name="T"/> and whose value is equivalent
        ///   to <paramref name="value"/>.</returns>
        public static T ChangeType<T>(object value, DeepConvertSettings settings) => (T)ChangeType(value, typeof(T), settings);
        ...
        public static object ChangeType(object value, Type destType, DeepConvertSettings settings)
        {
            ...
            if (typeof(IEnumerable<byte>).IsAssignableFrom(srcType) && destType == typeof(string))
            {
            ...
            if (destType == typeof(string) && typeof(IFormattable).IsAssignableFrom(srcType))
            {
            ...
            if (typeof(IDictionary).IsAssignableFrom(srcType) && destType.IsClass)
            {
                return ConvertToObject((IDictionary)value, destType, settings);
            }
            if (srcType.IsClass && typeof(IDictionary).IsAssignableFrom(destType))
            {
                var dict = ConvertToDictionary(value);
                return ChangeType(dict, destType, settings);
            }
            if (srcType.IsClass && destType.IsClass)
            {
                var dict = ConvertToDictionary(value);
        ...
        public static DateTime ToDateTime(object value) => ToDateTime(value, new DeepConvertSettings());
