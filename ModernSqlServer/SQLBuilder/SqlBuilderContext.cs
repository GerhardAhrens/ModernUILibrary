...
//
// <summary>
// Klasse zur Erstellung von SQL Anweisungen und die dazu passende SqlParameter
// </summary>
//-----------------------------------------------------------------------
...
namespace ModernSqlServer.Generator
{
    ...
    public sealed class SqlBuilderContext : IDisposable
    {
        ...
        /// </example>
        public (string, SqlParameter[]) GetInsert()
        {
            ...
            if (this.CurrentDataRow != null)
            {
                ...
                /* Oracle Parameter */
                prm = new SqlParameter[columnNames.Count];
                int step = 0;
                foreach (DataColumn column in columnNames)
                {
                    if (column.ColumnName.ToLower() == this.CreateByColumn.ToLower())
                    {
                        prm[step] = new SqlParameter(column.ColumnName, this.CurrentUser);
                    }
                    else if (column.ColumnName.ToLower() == this.CreateOnColumn.ToLower())
                    {
                        prm[step] = new SqlParameter(column.ColumnName, DateTime.Now);
                    }
                    else if (column.ColumnName.ToLower() == this.ModifyByColumn.ToLower())
                    {
                    }
                    ...
                    {
                        prm[step] = new SqlParameter(column.ColumnName, $"{DateTime.Now.ToString("dd.MM.yyyy HH:mm")} - {this.CurrentUser}");
                    }
                    else
                    {
                        prm[step] = new SqlParameter(column.ColumnName, this.CurrentDataRow[column]);
                    }
        ...
        public string GetSqlDump((string, SqlParameter[]) sql)
        {
            foreach (SqlParameter param in sql.Item2)
            {
        ...
        public (string, SqlParameter[]) GetUpdate()
        {
            StringBuilder sb = null;
            SqlParameter[] prm = null;
            string tableName = string.Empty;
        ...
        public (string, SqlParameter[]) GetDelete()
        {
            StringBuilder sb = null;
            SqlParameter[] prm = null;
            string tableName = string.Empty;
            ...
            if (this.CurrentDataRow != null)
            {
                ...
                prm[0] = new SqlParameter(this.KeyColumn.First(), this.CurrentDataRow[this.KeyColumn.First()]);
            }
