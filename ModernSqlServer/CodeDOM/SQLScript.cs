...
namespace ModernSqlServer.CodeDOM
{
    using System;
    ...
    public static partial class SQLScript
    {
        ...
        {
            DataTable table = DOMTable.FromClassInstanceCollection<T>(ClassObjects);
        ...
        {
            if (!CodeDOMHelper.IsValidDatatable(Table))
                return string.Empty;

            string columns = CodeDOMHelper.TableToColumnsString(Table);
            ...
            foreach (DataRow row in Table.Rows)
            {
                ...
                string values = CodeDOMHelper.RowToValueString(row);
        ...
        {
            if (!CodeDOMHelper.IsValidDatatable(Table, ignoreZeroRows: true))
                return string.Empty;
