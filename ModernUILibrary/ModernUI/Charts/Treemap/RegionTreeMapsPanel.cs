...
namespace ModernIU.Controls.Chart
{
    ...
    public class RegionTreeMapsPanel : TreeMapsPanel
    {
        ...
        {
            this.CalculatePanel(this.ManagedItems, new List<WeightUIElement>(), this.GetShortestSide());
        }
        ...
        private void CalculatePanel(List<WeightUIElement> items, List<WeightUIElement> row, double sideLength)
        {
            if (items.Count == 0)
            {
            ...
            double worstValue1 = this.WorstValue(row, sideLength);
            double worstValue2 = this.WorstValue(row2, sideLength);

            if (row.Count == 0 || worstValue1 > worstValue2)
                this.CalculatePanel(items2, row2, sideLength);
            else
            {
                this.AddRowToLayout(row);
                this.CalculatePanel(items, new List<WeightUIElement>(), this.GetShortestSide());
            }
        }
        ...
        private double WorstValue(List<WeightUIElement> row, double sideLength)
        {
            if (row.Count == 0)
                return 0;
