//-----------------------------------------------------------------------
// <copyright file="WrapPanel.cs" company="Lifeprojects.de">
//     Class: WrapPanel
...
namespace ModernIU.Controls
{
    ...
    public class WrapPanel : Panel
    {
        ...
        public static readonly DependencyProperty ItemHeightProperty = DependencyProperty.Register(
            "ItemHeight", typeof(double), typeof(WrapPanel), new PropertyMetadata(double.NaN, OnItemHeightOrWidthPropertyChanged));

        public static readonly DependencyProperty ItemWidthProperty = DependencyProperty.Register(
            "ItemWidth", typeof(double), typeof(WrapPanel), new PropertyMetadata(double.NaN, OnItemHeightOrWidthPropertyChanged));

        public static readonly DependencyProperty OrientationProperty = DependencyProperty.Register(
            "Orientation", typeof(Orientation), typeof(WrapPanel), new PropertyMetadata(Orientation.Vertical, OnOrientationPropertyChanged));
        ...
        private static void OnOrientationPropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            else
            {
                if (orientation != Orientation.Vertical && orientation != 0)
                {
                    ...
                    var text = string.Format(CultureInfo.InvariantCulture, "Exception thrown when the Orientation property is provided an invalid value.");
        ...
        private static void OnItemHeightOrWidthPropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            else
            {
                if (!num.IsNaN() && (num <= 0.0 || double.IsPositiveInfinity(num)))
                {
                    ...
                    var text = string.Format(CultureInfo.InvariantCulture, "Exception thrown when the ItemWith or ItemHeight properties are provided an invalid value.");
                    throw new ArgumentException(text, "value");
                }
        ...
        protected override Size ArrangeOverride(Size finalSize)
        {
            ...
            var flag2 = !itemHeight.IsNaN();
            var num = 0.0;
            var directDelta = (orientation == Orientation.Vertical) ? (flag ? itemWidth : default(double?)): (flag2 ? itemHeight : default(double?));
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                var orientedSize3 = new OrientedSize(orientation, flag ? itemWidth : uIElement.DesiredSize.Width, flag2 ? itemHeight : uIElement.DesiredSize.Height);
        ...
        private void ArrangeLine(int lineStart, int lineEnd, double? directDelta, double indirectOffset, double indirectGrowth)
        {
            ...
            {
                var uIElement = children[i];
                var orientedSize = new OrientedSize(orientation, uIElement.DesiredSize.Width, uIElement.DesiredSize.Height);
                var num2 = directDelta ?? orientedSize.Direct;
                var rect = flag ? new Rect(num, indirectOffset, num2, indirectGrowth) : new Rect(indirectOffset, num, indirectGrowth, num2);
